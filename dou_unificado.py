...
_IDEC_IRRELEVANT_PATTERNS = [
    re.compile(
        r"\bexped(?:ir|icao\s+de)\s+autorizacao\b.*\bcpf\b.*\bexplorar\s+servicos?\b",
        re.I,
    ),
    re.compile(
        r"\bexped(?:ir|icao\s+de)\s+autorizacao\b.*\bpessoa\s+fisica\b.*\bexplorar\s+servicos?\b",
        re.I,
    ),
    re.compile(
        r"\bexped(?:ir|icao\s+de)\s+autorizacao\b.*\bexplorar\s+servicos?\s+de\s+telecomunicacoes\s+de\s+interesse\s+restrito\b",
        re.I,
    ),
    re.compile(
        r"\breclamacao\s+individual\b.*\b(aneel|anatel|anpd|anvisa|ans|agencia\s+nacional)\b",
        re.I,
    ),
    re.compile(
        r"\bextincao\s+de\s+autorizac(?:ao|oes)\b.*\bservicos?\s+de\s+telecomunicacoes\s+de\s+interesse\s+restrito\b",
        re.I,
    ),
]
...
def _is_idec_irrelevante(texto_bruto: str) -> bool:
    nt = _normalize_ws(texto_bruto)
    return any(p.search(nt) for p in _IDEC_IRRELEVANT_PATTERNS)
...
FMCSV|Primeira infância|criança; criança feliz; alfabetização; creche; conanda; maternidade; parentalidade; paternidade; primeira infância; infantil; infância; infanto-juvenil; fundeb; educação básica; plano nacional de educação; homeschooling; FNDE; PNE; vaar; vaat; educação infantil; pré-escola; materno-infantil; infraestrutura escolar; política nacional de cuidados; bolsa família; visitação domiciliar
...
ASBAI|Saúde|alergia e imunologia; ASBAI; doenças imunoalérgicas; alergistas; imunologistas; atenção especializada; atenção primária; estratégia saúde da família; ESF; SUS; residência médica; bolsas de residência; formação médica; titulação médica; CNRM; comissão mista de especialidades; CME; SGTES; SAES; SAPS; PCDT; SVSA; anvisa; conitec; tabela SUS; linhas de cuidado; referência e contrarreferência; anafilaxia; choque anafilático; adrenalina autoinjetável; epinefrina autoinjetável; caneta de adrenalina; EpiPen; PL 85/2024; PL 793/2024; asma; rinite; alergia alimentar; dermatite atópica; urticária crônica; imunodeficiências; erros inatos da imunidade; doenças raras; vigilância epidemiológica; notificação compulsória; incorporação tecnológica; assistência farmacêutica; registro sanitário; regulação sanitária; inovação em saúde; ciência, tecnologia e inovação em saúde; complexo econômico-industrial da saúde; acesso a tecnologias em saúde
Infinis|Primeira infância|primeira infância; conanda; criança; ECA; SUAS; estatuto da criança e do adolescente; Infinis; saúde mental; RAPS; violência contra crianças; violência psicológica; saúde indígena; plano nacional de enfrentamento da violência contra crianças e adolescentes; protagonismo juvenil; coalizão brasileira pelo fim da violência contra crianças e adolescentes; violência sexual contra crianças; sistema único de assistência social; pesquisa nacional de saúde mental; adolescente; insegurança alimentar; orçamento primeira infância; assistência social; saúde mental indígena
...
def procura_termos_clientes(conteudo_raspado: dict | None, campo_secao: str = "secao") -> dict[str, list]:
    ...
    for r in conteudo_raspado["jsonArray"]:
        ...
        for cliente, kw in hits:
            if cliente == "IDEC" and _is_idec_irrelevante(alltxt):
                continue
            key = (cliente, link)
            if key not in agreg:
                agreg[key] = {
